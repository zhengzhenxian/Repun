...
def output_vcf_header(reference_file_path, output_fn=None, sample_name='SAMPLE'):
    ...
    header += dedent("""\
        ...
        ##FORMAT=<ID=ED,Number=1,Type=Integer,Description="Edit distance between truth haplotypes and candidate haplotypes">"""
                  ) + '\n'
...
def sort_vcf_from(args):
    ...
    truth_dict = defaultdict()
    if args.truth_vcf_fn is not None:
        with open(args.truth_vcf_fn) as f:
            for row in f:
                if row[0] == '#' or not len(row.rstrip()):
                    continue
                columns = row.strip('\t').split(maxsplit=3)
                ctg_name, pos = columns[0], columns[1]
                truth_dict[(ctg_name, int(pos))] = row
    ...
    for contig in contigs_order_list:
        ...
        for vcf_fn in contig_vcf_fns:
            ...
            if is_lz4_format:
                read_proc.wait()
        if need_write_header and len(vcf_header):
            #add cmdline for gvcf
            if "##cmdline" not in '\n'.join(vcf_header) and cmd_fn is not None and os.path.exists(cmd_fn):
                cmdline_str = ""
                if cmd_fn is not None and os.path.exists(cmd_fn):
                    ...
                    if cmd_line is not None and len(cmd_line) > 0:
                        cmdline_str = "##cmdline={}\n".format(cmd_line)
                if cmdline_str != "":
                    insert_index = 3 if len(vcf_header) >= 3 else len(vcf_header) - 1
        ...
        for pos in all_pos:
            if pos in contig_dict:
                row = contig_dict[pos]
                if (contig, pos) in truth_dict:
                    truth_row = truth_dict[(contig, pos)]
                    dp, af = truth_row.rstrip().split('\t')[9].split(':')[0:2]
                    unified_row = contig_dict[pos]

                    if args.vaf_threshold_for_pass is not None:
                        vaf = float(af)
                        if vaf > float(args.vaf_threshold_for_pass):
                            unified_row = unified_row.rstrip().replace("LowVAF", "PASS")
                        elif 'U' not in unified_row.rstrip().split('\t')[7]:
                            unified_row = unified_row.rstrip().replace("PASS", "LowVAF")

                    columns = unified_row.rstrip().split('\t')
                    if 'U' in columns[7]:
                        unified_count += 1
                ...
                if args.vaf_threshold_for_pass is not None:
                    ...
                    vaf = float(af)
                    if vaf > float(args.vaf_threshold_for_pass):
                        row = row.rstrip().replace("LowVAF", "PASS")
...
def main():
    ...
    parser.add_argument('--vaf_threshold_for_pass', type=str, default=None,
                        help="If set, variants with >VAF will be marked as PASS, or LowVAF otherwise")
