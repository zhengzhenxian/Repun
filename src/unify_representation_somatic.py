...
def align_sequences(seq1, seq2, gap_char='-'):
    ...
    for a, b in zip(align1, align2):
        match_line.append('|' if a == b and a != gap_char else ' ')
    match_line = ''.join(match_line)

    print('\n'.join([align1, match_line, align2]))


def compute_edit_distance(seq1, seq2, gap_open_penalty=2, gap_extend_penalty=1, remove_gap=False, add_match_line=True):
    ...
    for i in range(len1 + 1):
        dp[i][0] = i * gap_open_penalty
    for j in range(len2 + 1):
        dp[0][j] = j * gap_open_penalty
    ...
    for i in range(1, len1 + 1):
        for j in range(1, len2 + 1):
            if seq1[i - 1] == seq2[j - 1]:
                ...
                dp[i][j] = min(
                    dp[i - 1][j] + gap_open_penalty,
                    dp[i][j - 1] + gap_open_penalty,
                    dp[i - 1][j - 1] + 1
                )

            if i > 1:
                dp[i][j] = min(dp[i][j], dp[i - 1][j] + gap_extend_penalty)
            if j > 1:
                dp[i][j] = min(dp[i][j], dp[i][j - 1] + gap_extend_penalty)
    ...
    while i > 0 or j > 0:
        if i > 0 and j > 0 and seq1[i - 1] == seq2[j - 1]:
            ...
            j -= 1
                ...
                j == 0 or dp[i][j] == dp[i - 1][j] + gap_open_penalty or dp[i][j] == dp[i - 1][j] + gap_extend_penalty):
            alignment1.append(seq1[i - 1])
            alignment2.append('-')
            i -= 1
        elif j > 0 and (
                i == 0 or dp[i][j] == dp[i][j - 1] + gap_open_penalty or dp[i][j] == dp[i][j - 1] + gap_extend_penalty):
            alignment1.append('-')
            alignment2.append(seq2[j - 1])
            j -= 1
        else:
    ...
    info.append("".join(match_line))
    info.append(alignment2)
    info.append(f"Edit Distance: {dp[len1][len2]}")
    print("".join(alignment2))
    print(f"Edit Distance: {dp[len1][len2]}")
    print(info)
    return '\n'.join(info)

def is_strict_subsequence(A, B):
    i = j = 0
...
def edit_distance(s1, s2, return_signed=False):
    swapped = len(s1) > len(s2)
    if swapped:
        s1, s2 = s2, s1
    ...
    if return_signed:
        return distances[-1] * (-1 if swapped else 1)
    return distances[-1]
...
class ReadMatch(object):
    def __init__(self, sample_ctg_info, candidates, candidate_genotypes, truths,match_seq,truth_genotypes,match_reads_count=0, edit_distance=None, truth_seq=None,best_sorted_edit_pair=None, match_info_list=None):
        ...
        self.truth_seq = truth_seq
        self.match_order = (self.edit_distance, self.match_reads_count, self.miss_variants_count, self.non_variants_count, self.match_variants_count)
        self.best_sorted_edit_pair = best_sorted_edit_pair
        self.match_info_list = match_info_list

    def return_match_info(self):
        if self.match_info_list is None:
            return ""
        [c_ref_seq, c_alt_seq, t_ref_seq, t_alt_seq] = self.match_info_list
        output_info = ['Candidates match:']
        print()
        output_info.append(align_sequences(c_ref_seq, c_alt_seq))
        output_info.append('Truths match:')
        output_info.append(align_sequences(t_ref_seq.replace('-', ''), t_alt_seq.replace('-', '')))
        output_info.append('Match between candidate and truths:')
        output_info.append(compute_edit_distance(c_alt_seq, t_alt_seq, remove_gap=True))
        return '\n'.join(output_info)
    ...
    def match_info(self):
        ...
        extro_info = ""
        if self.match_reads_count >=-3 and self.miss_variants_count > 0 and self.match_variants_count > 0:
            extro_info = '\nthis match has few read support'
        return ('ctg_info={}, editing_distance,read_support,miss_variants,non_variants,match_variants={}, candidate={}, truth={} {}').format(self.sample_ctg_info,
            self.match_order,
...
def cal_depth(alt_list):
    return sum([int(item[1]) for item in alt_list if item[0] not in 'ID'])


def cal_ad(pos_alt_dict, ref_base, alt_base):
    if pos_alt_dict is None or len(pos_alt_dict) == 0:
        return 0
    if len(ref_base) == 1 and len(alt_base) == 1:
        key = 'X' + alt_base.upper()
        return pos_alt_dict[key] if key in pos_alt_dict else 0
...
class RepresentationUnification(object):
    ...
    def __init__(self,
                 ...
                 max_calculate_count,
                 subsample_ratio,
                 find_match_with_most_read_support=False):
        ...
        self.sample_ctg_info = contig_name
        self.partition_size = partition_size
        self.max_candidates_distance = max_candidates_distance
        self.max_calculate_count = max_calculate_count
        self.reference_sequence = reference_sequence
        self.reference_start = reference_start
        self.find_match_with_most_read_support = find_match_with_most_read_support
    ...
    def find_match_pairs(self, candidates, truths, ref, variant_dict, read_name_info_dict=None, alt_dict=None):
        ...
        if no_match_found:
            ...
            print ('[INFO] Missing match: ctg_info={}, editing_distance,read_support,miss_variants,non_variants,match_variants=(-1, 0, {}, {}, 0), candidate={}, truth={}'.format(self.sample_ctg_info,len(truths), len(candidates),can_info, truth_info))
            return None
        ...
        if confident_match:
            ...
            print (
                '[INFO] Found confident match: ctg_info={}, editing_distance,read_support,miss_variants,non_variants,match_variants=(None, 0, 0, {}), candidate={}, truth={}'.format(
        ...
        def get_snv_and_indel(truths, candidates):
            truth_snv_count = 0
            truth_indel_count = 0
            for truth in truths:
                if len(truth.reference_bases) == 1 and len(truth.alternate_bases[0]) == 1:
                    truth_snv_count += 1
                else:
                    truth_indel_count += 1
            candidate_snv_count = 0
            candidate_indel_count = 0
            for candidate in candidates:
                if len(candidate.reference_bases) == 1 and len(candidate.alternate_bases[0]) == 1:
                    candidate_snv_count += 1
                else:
                    candidate_indel_count += 1
            return truth_snv_count, truth_indel_count, candidate_snv_count, candidate_indel_count

        truth_snv_count, truth_indel_count, candidate_snv_count, candidate_indel_count = get_snv_and_indel(truths, candidates)
        matches = []
        count = 0
        for variant_seq, variant_genotypes in variant_seqs.items():
            #rank by supporting count
            if len(variant_seq) == 1 and list(variant_seq)[0] == ref.seq:
                continue
            match_reads_count = -sum(
                [read_seqs_counter[seq] if read_seqs_counter and seq in read_seqs_counter else 0 for seq in
                 variant_seq])
            if match_reads_count == 0:
                count += 1
                continue
            ...
            edit_pair= []
            for seq in seqs:
                if len(seq) == 1 and list(seq)[0] == ref.seq:
                    continue
                if seq1 == seq:
                    edit_pair.append([seq,0])
                else:
                    if len(seq1) > 1:
                        seq11, seq12 = seq1
                        #move ref seq to first position if exist
                        if seq11 != ref.seq and seq12 == ref.seq:
                            seq12, seq11 = seq1
                    else:
                        seq11 = seq12 = list(seq1)[0]
                    if len(seq) > 1:
                        seq21, seq22 = seq
                        if seq21 != ref.seq and seq22 == ref.seq:
                            seq22, seq21 = seq
                    else:
                        seq21 = seq22 = list(seq)[0]

                    #compare the seq with alternate bases
                    if seq11 == ref.seq and seq21 == ref.seq:
                        match_11_22 = edit_distance(seq12, seq22, return_signed=True)
                        if not self.args.allow_candidate_haplotype_shorter_than_truth_haplotype and not is_strict_subsequence(seq12, seq22):
                            match_11_22 = len(ref.seq) + 1
                        else:
                            match_11_22 = math.fabs(match_11_22)
                        match_12_21 = len(ref.seq) + 1
                    else:

                        match_11 = edit_distance(seq11, seq21, return_signed=True)
                        match_22 = edit_distance(seq12, seq22, return_signed=True)
                        if not self.args.allow_candidate_haplotype_shorter_than_truth_haplotype and not is_strict_subsequence(seq11, seq21):
                            match_11 = len(ref.seq) + 1
                        if not self.args.allow_candidate_haplotype_shorter_than_truth_haplotype and not is_strict_subsequence(seq12, seq22):
                            match_22 = len(ref.seq) + 1
                        match_11_22 = math.fabs(match_11) + math.fabs(match_22)

                        match_12 = edit_distance(seq11, seq22, return_signed=True)
                        match_21 = edit_distance(seq12, seq21, return_signed=True)
                        if not self.args.allow_candidate_haplotype_shorter_than_truth_haplotype and not is_strict_subsequence(seq11, seq22):
                            match_12 = len(ref.seq) + 1
                        if not self.args.allow_candidate_haplotype_shorter_than_truth_haplotype and not is_strict_subsequence(seq12, seq21):
                            match_21 = len(ref.seq) + 1

                        match_12_21 = math.fabs(match_12) + math.fabs(match_21)

                    minimum_edit = min(match_11_22, match_12_21)

                    if truth_snv_count > 0 and truth_indel_count > 0:
                        if minimum_edit > self.args.snv_maximum_edit_distance + self.args.indel_maximum_edit_distance:
                            continue
                    else:
                        if truth_snv_count > 0 and minimum_edit > self.args.snv_maximum_edit_distance:
                            continue
                        if truth_indel_count > 0 and minimum_edit > self.args.indel_maximum_edit_distance:
                            continue
                    edit_pair.append([seq, minimum_edit, match_11_22, match_12_21])
            if len(edit_pair) == 0:
                continue
            sorted_edit_pair = sorted(edit_pair, key=lambda x: x[1])

            best_match_truth_seq = sorted_edit_pair[0][0]
            best_minimum_edit = sorted_edit_pair[0][1]

            truth_seq = truth_seqs[best_match_truth_seq]
            if len(truth_seq) == 0:
                continue
            for variant_genotype in variant_genotypes:
                 # more match reads and negative count is better if smaller
                matches.append(ReadMatch(
                        ...
                        truth_seq=best_match_truth_seq,
                        match_reads_count=match_reads_count,
                        best_sorted_edit_pair=sorted_edit_pair[0],
                        edit_distance=sorted_edit_pair[0][1]))
        if not matches:
            return None
        else:
            ...
            #??
            if len(best_matches.best_sorted_edit_pair) < 4:
                genotype_index = 1
            else:
                genotype_index = 1 if best_matches.best_sorted_edit_pair[2] <= best_matches.best_sorted_edit_pair[3] else 0

            candidates = best_matches.candidates
            truths = best_matches.truths

            candidates_dict = {candidate.start: candidate for candidate in candidates}
            candidate_genotype_dict = {candidate.start: gt for candidate, gt in zip(candidates, best_matches.candidate_genotypes)}
            truth_genotype_dict = {candidate.start: gt for candidate, gt in zip(truths, best_matches.truth_genotypes)}
            print("Candidate variants:")
            for candidate, gt in zip(candidates, best_matches.candidate_genotypes):
                print(self.args.ctgName, candidate.pos, candidate.reference_bases+ '->' + ','.join(candidate.alternate_bases), '/'.join(map(str, gt)))
            print("Truth variants:")
            for candidate, gt in zip(truths, best_matches.truth_genotypes):
                print(self.args.ctgName, candidate.pos, candidate.reference_bases+ '->' + ','.join(candidate.alternate_bases), '/'.join(map(str, gt)))

            flanking = 5
            start_pos_list = [candidate.pos for candidate in candidates] + [candidate.pos for candidate in truths]
            end_pos_list = [candidate.pos + max(len(candidate.reference_bases), len(candidate.alternate_bases[0])) for candidate in candidates] + [candidate.pos + max(len(candidate.reference_bases), len(candidate.alternate_bases[0])) for candidate in truths]
            ref_start = max(ref.start, min(start_pos_list) - flanking)
            ref_end = min(ref.end, max(end_pos_list) + flanking)

            def get_match_seqs(ref, candidates_dict, candidate_genotype_dict, ref_start, ref_end, genotype_index=1):

                ref_seq = []
                alt_seq = []
                p = ref_start
                while p < ref_end:
                    if p in candidates_dict:
                        ref_base = candidates_dict[p].reference_bases
                        alt_base = candidates_dict[p].alternate_bases[0]
                        gt = candidate_genotype_dict[p]

                        if sum(gt) <= 0 or (gt[genotype_index] == 0):
                            ref_seq.append(ref.query(p, p + 1))
                            alt_seq.append(ref.query(p, p + 1))
                            p += 1
                            continue
                        #del
                        if len(ref_base) > 1 and len(alt_base) == 1:
                            ref_seq.append(ref_base)
                            alt_seq.append(alt_base + '-' * (len(ref_base) -1))
                            p += len(ref_base)
                        elif len(ref_base) == 1 and len(alt_base) > 1:
                            ref_seq.append(ref_base + '-' * (len(alt_base) -1))
                ...
                return ''.join(ref_seq), ''.join(alt_seq)
            truths_dict = {candidate.start: candidate for candidate in truths}
            c_ref_seq, c_alt_seq = get_match_seqs(ref, candidates_dict, candidate_genotype_dict, ref_start=ref_start, ref_end=ref_end, genotype_index=genotype_index)
            t_ref_seq, t_alt_seq = get_match_seqs(ref, truths_dict, truth_genotype_dict, ref_start=ref_start, ref_end=ref_end, genotype_index=genotype_index)
            print('Reference-Candidates match:')
            align_sequences(c_ref_seq, c_alt_seq)
            print()
            print('Reference-Truths match:')
            align_sequences(t_ref_seq, t_alt_seq)
            print()
            print('Candidates-Truths match:')
            align_sequences(c_alt_seq, t_alt_seq)
            print()
            best_matches.match_info_list= [c_ref_seq, c_alt_seq, t_ref_seq, t_alt_seq]
            return best_matches


    def unify_label(self, variants, truths, region, ctg_start, ctg_end, all_pos, variant_dict,
                  rescue_dict=None, output_vcf_fn=None, test_pos=None, read_name_info_dict=None, alt_dict=None, output_info_fn=None):
        ...
        for all_candidates, all_truths in all_partitions:
            ...
            for idx, (candidate, candidate_genotypes) in enumerate(
                ...
                pos = candidate.start
                alt_list = alt_dict[pos].alt_list
                pos_alt_dict = {item[0]: item[1] for item in alt_list}

                if sum(candidate_genotypes) == 0:
                    continue
                candidate_depth = cal_depth(alt_dict[pos].alt_list)
                candidate_allele_depth = cal_ad(pos_alt_dict, candidate.reference_bases, candidate.alternate_bases[0])
                try:
                    candidate_af = candidate_allele_depth / float(candidate_depth) if candidate_depth > 0 else 0.0
...
def UnifyRepresentation(args):
    ...
    if candidate_details_fn is None:
        ...
        CTFA_Bin = basedir + "/../ru.py extract_candidates_somatic"
        ...
        create_tensor_command_options = [
            ...
            CommandOption('samtools', args.samtools),
            ctgStart,
            ctgEnd,
            chunkId,
            chunkNum,
    ...
    if args.output_info_fn:
        output_info_fn = open(args.output_info_fn, "w")
    ...
    for split_idx in range((region_end - region_start) // region_size + 1):
        ...
        RU = RepresentationUnification(
            ...
            find_match_with_most_read_support=args.find_match_with_most_read_support)
...
def main():
    ...
    parser.add_argument('--output_info_fn', type=str, default=None,
                        help="VCF output filename or stdout if not set,default: %(default)s")
    ...
    parser.add_argument('--find_match_with_most_read_support', type=str2bool, default=None,
                        help="The 1-based starting position of the sequence to be processed, optional, will process the whole --ctgName if not set")
    ...
    ## The number of chucks to be divided into for parallel processing
    parser.add_argument('--snv_maximum_edit_distance', type=int, default=0,
                        help="Maximum SNV edit distance that allow to be unified, default: 0")

    parser.add_argument('--indel_maximum_edit_distance', type=int, default=4,
                        help="Maximum Indel edit distance that allow to be unified, default: 0")

    parser.add_argument('--allow_candidate_haplotype_shorter_than_truth_haplotype', type=str2bool, default=None,
                        help="Allow the candidate haplotype shorter than truth haploytpe, default: False")

    parser.add_argument('--minimum_supporting_reads', type=int, default=None,
                        help=SUPPRESS)

    parser.add_argument('--minimum_supporting_af', type=float, default=None,
                        help=SUPPRESS)
